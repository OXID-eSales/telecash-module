...
docker compose run php ${OE_CONSOLE} oe:module:deactivate osc_telecash
docker compose run php ${OE_CONSOLE} oe:module:activate osc_telecash
...
NETWORK_NAME="telecash7-jan14_default"
...
if lsof -i :5900 >/dev/null 2>&1; then
    echo "Port 5900 is in use. Please free the port to continue."
    sudo lsof -i :5900
    exit 1
fi

echo "Cleaning up existing network..."
docker network rm telecash7-jan14_default || echo "Network not found, continuing..."
